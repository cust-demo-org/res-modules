...
- [random_string.cosmos_db_suffix](https://registry.terraform.io/providers/hashicorp/random/latest/docs/resources/string) (resource)
...
### <a name="input_cosmos_db"></a> [cosmos\_db](#input\_cosmos\_db)

Description: A map of Azure Cosmos DB (DocumentDB) Database Accounts to create using the `Azure/avm-res-documentdb-databaseaccount/azurerm` module.  
...
Description: Key Vaults output map from spoke module. Used to resolve customer\_managed\_key.key\_vault\_key to a Key Vault resource ID.
...
Description: Map of Log Analytics workspace keys to their resource IDs. Used to resolve diagnostic\_settings workspace\_key / use\_default\_log\_analytics.
...
Description: Managed identities output map from spoke module. Used to resolve user\_assigned\_keys, role\_assignments managed\_identity\_key, and customer\_managed\_key user\_assigned\_identity.key to UAMI resource IDs / principal IDs.
...
Description: Combined private DNS zones resource-ID map from spoke module (combined\_private\_dns\_zones\_resource\_ids). Used to resolve private\_endpoints[*].private\_dns\_zone.keys to DNS zone resource IDs.
...
Description: Virtual networks output map from spoke module. Used to resolve vnet\_key/subnet\_key (private endpoints and virtual\_network\_rules) to a subnet resource ID.
...
Description: Map of Cosmos DB account keys to their AVM module objects. Each object includes resource\_id, name, endpoint, and the module's other outputs (sql\_databases, mongo\_databases, cosmosdb\_keys, connection strings, etc.).
...
This module's output (`cosmos_db`) can be consumed by other modules in this repository.
<!-- END_TF_DOCS -->