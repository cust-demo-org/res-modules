...
Description: Log Analytics workspaces output map from the spoke module. Used to resolve diagnostic\_settings.workspace\_key to a workspace resource ID.
...
Description: A map of Azure Public IP Addresses to create using the `Azure/avm-res-network-publicipaddress/azurerm` AVM module.  
...
Description: Map of public IP address keys to their AVM module objects (includes resource\_id, public\_ip\_id, public\_ip\_address, name, and other module outputs).
...
This module's output (`public_ips`) can be consumed by other modules in this repository. The `application_gateway` and `api_management_service` modules reference public IPs via their map keys.
<!-- END_TF_DOCS -->